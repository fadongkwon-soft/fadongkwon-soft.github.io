...
2. **링크(`href`)와 이미지 경로는 한국어 원문 그대로 둔다.** `/ko/posts/...` → `/posts/...`
...
**예약하라는 지시가 따로 없으면 현재 시각으로 올린다.** 파일명 날짜와 `date` 를 모두 지금 시각으로 쓰고,
이미지 폴더 `assets/img/<YYYYMMDD>_<slug>/` 도 같은 날짜로 맞춘다.
기존 글이 미래 날짜로 예약돼 있다고 해서 그 뒤에 얹지 말 것 — 예약 큐는 예약 지시가 있을 때만 본다.

---
title: Pixel Pong — One-on-One Against the AI or a Friend
description: Pong with three AI difficulties, local two-player on one phone and an arcade mode with power-ups you can switch off, now on Google Play and Apps in Toss
image:
  path: /assets/img/20260918_pixel-pong/feature.png
  alt: Pixel Pong feature graphic
date: 2026-09-18 09:00:00 +0900
categories: [Products, Game]
permalink: /en/posts/pixel-pong/
alt_url: /posts/pixel-pong/
tags: [game, android, arcade, two player, solo developer]
---
## Info
> **Pixel Pong** is one-on-one pong: three AI difficulties, local two-player on a single phone, and an arcade mode with power-ups you can turn off. Works offline, no sign-up. 9 languages.
{: .prompt-info }

## Two People, One Phone
Each player takes one end of the screen and faces the other. Alone, the AI takes a side.
**Paddles move by dragging sideways**, so your finger never sits on top of the ball.

- 🎚️ **Three AI difficulties** — Easy · Normal · Hard
- 👥 **Two-player** — put one phone between you and sit facing each other
- 🕹️ **Arcade mode** — power-ups fly in mid-rally
- 🎁 **Power-ups on or off** — turn them off if you want a straight match
- 🏅 **Record** — wins, losses and best scores accumulate
- 📤 **Share** — turn a result into an image and send it to a friend

It works fully offline with no sign-up or login, and supports 9 languages.

![Pixel Pong home screen](/assets/img/20260918_pixel-pong/shot-home.png){: w="360" }
![Gameplay screen](/assets/img/20260918_pixel-pong/shot-play.png){: w="360" }

## Download
- Google Play: <https://play.google.com/store/apps/details?id=com.fadongkwon.pixel_pong>
- Apps in Toss: <https://fadongkwon.com/toss/pixel-pong/> — open it on your phone and it connects straight into the Toss app.

## Released Alongside
Ten games launched together on September 8, and I am introducing one a day. Published so far:

- [2048](/en/posts/game-2048/) — Slide and Merge Your Way to the 2048 Tile
- [Minesweeper](/en/posts/minesweeper/) — Read the Numbers, Avoid the Mines
- [Solitaire](/en/posts/solitaire/) — The Klondike You Already Know
- [Brick Breaker](/en/posts/breakout/) — Bounce, Break, Repeat
- [Gomoku](/en/posts/gomoku/) — First to Five in a Row
- [Snake](/en/posts/snake/) — Eat, Grow, Try Not to Bite Yourself
- [Number Slide](/en/posts/slide-puzzle/) — The 15 Puzzle, With Pictures

The story behind the previous six is in [Shipping Six Mini Games at Once](/en/posts/six-games-retrospective/).

I post updates here and on [Instagram (@fadongkwon.soft)](https://www.instagram.com/fadongkwon.soft/).

---
title: 숫자 퍼즐 — 타일을 밀어 순서대로 맞추는 15퍼즐
description: 고전 15퍼즐에 그림 모드와 완성 연출, 오늘의 도전과 이어하기를 담은 숫자 퍼즐을 Google Play와 앱인토스에 출시했습니다
date: 2026-09-17 09:00:00 +0900
categories: [Products, Game]
tags: [game, android, 숫자퍼즐, 15퍼즐, 퍼즐, 1인개발자]
pin: false
image:
  path: /assets/img/20260917_slide-puzzle/feature.png
alt_url: /en/posts/slide-puzzle/
---
## Info
> **Number Slide** is the classic 15 puzzle: slide tiles into the gap to put them in order. Picture mode, a completion reveal, a daily board and auto-save. Works offline, no sign-up. 9 languages.
{: .prompt-info }

## 숫자로도, 그림으로도
빈칸으로 타일을 밀어 순서를 맞추는 그 퍼즐입니다.
숫자가 지겨워지면 **그림 모드**로 바꿀 수 있고, 다 맞추면 완성된 그림을 먼저 보여줍니다.

- 🔢 **고전 15퍼즐** — 빈칸으로 타일을 밀어 정렬합니다
- 🖼️ **그림 모드** — 숫자 대신 그림 조각을 맞춥니다. 썸네일을 눌러 크게 볼 수 있어요
- 🎉 **완성 연출** — 마지막 조각을 놓으면 완성된 그림을 먼저 보여줍니다
- 📅 **오늘의 도전** — 매일 새 판, 기록이 이어집니다
- 🏅 **기록** — 걸린 시간과 이동 수가 쌓입니다
- 💾 **이어하기** — 진행 중인 판은 자동 저장됩니다

완전 오프라인으로 동작하고, 회원가입·로그인 없이 바로 시작할 수 있습니다. 9개 언어를 지원합니다.

![숫자 퍼즐 홈](/assets/img/20260917_slide-puzzle/shot-home.png){: w="360" }
![플레이 화면](/assets/img/20260917_slide-puzzle/shot-play.png){: w="360" }

## Download
- Google Play: <https://play.google.com/store/apps/details?id=com.fadongkwon.slide_puzzle>
- 앱인토스(토스 미니앱): <https://fadongkwon.com/toss/slide-puzzle/> — 휴대폰에서 열면 토스 앱으로 바로 연결됩니다.

## 함께 나온 게임
9월 8일에 10종을 함께 출시했고, 하루에 하나씩 소개하고 있습니다. 지금까지 올린 글입니다.

- [2048](/posts/game-2048/) — 숫자를 합쳐 2048 타일을 만드는 퍼즐
- [지뢰찾기](/posts/minesweeper/) — 숫자를 읽고 지뢰를 피하는 논리 퍼즐
- [솔리테어](/posts/solitaire/) — 윈도우에서 하던 그 클론다이크
- [벽돌깨기](/posts/breakout/) — 공을 튕겨 벽돌을 모두 깨는 아케이드
- [오목](/posts/gomoku/) — 다섯 알을 먼저 잇는 사람이 이깁니다
- [뱀 게임](/posts/snake/) — 먹이를 먹고 길어지는 클래식

먼저 나온 6종의 이야기는 [미니게임 6종 동시 출시 회고](/posts/six-games-retrospective/)에 정리해 두었습니다.

소식은 이 블로그와 [인스타그램(@fadongkwon.soft)](https://www.instagram.com/fadongkwon.soft/)에서 전해드립니다.

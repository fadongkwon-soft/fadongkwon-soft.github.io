---
title: 가위바위보 — 패턴을 읽는 AI와 연승 대결
description: 운으로 겨루는 랜덤과 내 손버릇을 읽는 고수 두 단계 AI, 한 폰으로 마주 앉는 2인 대결, 난이도별 연승 기록을 담은 가위바위보를 Google Play와 앱인토스에 출시했습니다
date: 2026-09-21 09:00:00 +0900
categories: [Products, Game]
tags: [game, android, 가위바위보, 캐주얼, 대전게임, 1인개발자]
pin: false
image:
  path: /assets/img/20260921_rock-paper-scissors/feature.png
alt_url: /posts/rock-paper-scissors/
---
## Info
> **Rock Paper Scissors Streak** has two AI levels: Random plays on pure luck, Master reads the patterns in your hands. Or hand the phone to a friend and play face to face, first to 3 wins. TOP 10 streaks per level, works offline, no sign-up. 9 languages.
{: .prompt-info }

## 규칙은 다 아는데, 상대가 나를 읽습니다
구호에 맞춰 가위·바위·보 중 하나를 고릅니다. 여기까지는 설명이 필요 없습니다.
다만 **고수**는 내가 지금까지 낸 손을 기억하고 다음 수를 읽습니다.
사람은 생각보다 같은 손을 반복하고, 진 다음에는 손을 바꾸는 버릇이 있습니다.

- 🎲 **랜덤** — 순수하게 운으로 겨루는 상대
- 🧠 **고수** — 내 패턴을 읽고 받아치는 상대
- 👥 **둘이서 대결** — 한 기기를 가운데 놓고 마주 앉아 먼저 3승
- 🏅 **연승 기록 TOP 10** — 난이도별로 따로 쌓입니다
- 📊 **전적과 승률** — 승·무·패가 함께 기록됩니다
- 📣 **연승 공유** — 친구에게 도전장을 보낼 수 있어요

둘이서 대결할 때는 고른 손이 화면에 표시되지 않습니다.
맞은편에서 보이면 게임이 안 되니, 둘 다 고른 뒤에 동시에 공개합니다.

완전 오프라인으로 동작하고, 회원가입·로그인 없이 바로 시작할 수 있습니다. 9개 언어를 지원합니다.

![가위바위보 홈 화면](/assets/img/20260921_rock-paper-scissors/shot-home.png){: w="360" }
![대결 화면](/assets/img/20260921_rock-paper-scissors/shot-play.png){: w="360" }

## 플레이 영상
{% include embed/youtube.html id='z0Zf7I5pq7k' %}

## Download
- Google Play: <https://play.google.com/store/apps/details?id=com.fadongkwon.rock_paper_scissors>
- 앱인토스(토스 미니앱): <https://fadongkwon.com/toss/rock-paper-scissors/> — 휴대폰에서 열면 토스 앱으로 바로 연결됩니다.

## 함께 나온 게임
9월 8일 Google Play에 13종을 함께 냈습니다. 그중 10종은 앱인토스에도 같이 올라가 하루에 하나씩 소개해 왔고, 이 게임을 포함한 나머지 3종은 앱인토스 심사가 늦어져 9월 13일에 합류했습니다. 이제 마저 소개합니다.

- [2048](/ko/posts/game-2048/) — 숫자를 합쳐 2048 타일을 만드는 퍼즐
- [지뢰찾기](/ko/posts/minesweeper/) — 숫자를 읽고 지뢰를 피하는 논리 퍼즐
- [솔리테어](/ko/posts/solitaire/) — 윈도우에서 하던 그 클론다이크
- [벽돌깨기](/ko/posts/breakout/) — 공을 튕겨 벽돌을 모두 깨는 아케이드
- [오목](/ko/posts/gomoku/) — 다섯 알을 먼저 잇는 사람이 이깁니다
- [뱀 게임](/ko/posts/snake/) — 먹이를 먹고 길어지는 클래식
- [숫자 퍼즐](/ko/posts/slide-puzzle/) — 타일을 밀어 순서대로 맞추는 15퍼즐
- [픽셀 탁구](/ko/posts/pixel-pong/) — AI 또는 친구와 1:1 탁구 대결
- [타워 쌓기](/ko/posts/tower-stack/) — 타이밍 맞춰 블록을 높이 쌓기
- [하늘 점프](/ko/posts/sky-jump/) — 발판을 밟고 끝없이 높이 오르기

먼저 나온 6종의 이야기는 [미니게임 6종 동시 출시 회고](/ko/posts/six-games-retrospective/)에 정리해 두었습니다.

소식은 이 블로그와 [인스타그램(@fadongkwon.soft)](https://www.instagram.com/fadongkwon.soft/)에서 전해드립니다.

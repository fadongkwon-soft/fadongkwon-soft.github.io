---
title: 두더지 잡기 — 60초 동안 튀어나오는 두더지를 톡톡
description: 두더지는 +1, 황금 두더지는 +3, 폭탄은 -2. 3단계 난이도와 콤보, 새싹 농부부터 두더지 왕까지 6단계 배지를 담은 두더지 잡기를 Google Play와 앱인토스에 출시했습니다
date: 2026-09-22 09:00:00 +0900
categories: [Products, Game]
tags: [game, android, 두더지잡기, 순발력, 아케이드, 1인개발자]
pin: false
image:
  path: /assets/img/20260922_whack-a-mole/feature.png
alt_url: /posts/whack-a-mole/
---
## Info
> **Mole Tap** gives you 60 seconds. A mole is +1, a gold mole is +3, and a bomb takes 2 away. Three difficulties, a combo counter for consecutive hits, six rank badges and TOP 10 scores per difficulty. Works offline, no sign-up. 9 languages.
{: .prompt-info }

## 60초, 그 안에 몇 마리나 잡을까요
구멍에서 두더지가 튀어나옵니다. 손가락으로 톡 치면 점수가 오릅니다.
규칙은 그게 전부인데, 급해지면 손이 먼저 나갑니다. 폭탄까지 두드리게 됩니다.

- 🐹 **두더지 +1** — 기본 점수
- ✨ **황금 두더지 +3** — 반짝일 때 놓치면 아깝습니다
- 💣 **폭탄 -2** — 급할수록 눈에 안 들어옵니다
- 🎚️ **3단계 난이도** — 쉬움 · 보통 · 어려움
- 🔥 **콤보** — 연속으로 잡으면 카운트가 올라갑니다
- 🏅 **6단계 배지** — 새싹 농부부터 두더지 왕까지
- 🧾 **결과 카드** — 점수·최대 콤보·잡은 두더지·황금·폭탄이 한 장에

난이도별로 기록 TOP 10이 따로 쌓이고, 점수를 공유해 친구에게 도전장을 보낼 수 있습니다.

완전 오프라인으로 동작하고, 회원가입·로그인 없이 바로 시작할 수 있습니다. 9개 언어를 지원합니다.

![두더지 잡기 홈 화면](/assets/img/20260922_whack-a-mole/shot-home.png){: w="360" }
![플레이 화면](/assets/img/20260922_whack-a-mole/shot-play.png){: w="360" }

## Download
- Google Play: <https://play.google.com/store/apps/details?id=com.fadongkwon.whack_a_mole>
- 앱인토스(토스 미니앱): <https://fadongkwon.com/toss/whack-a-mole/> — 휴대폰에서 열면 토스 앱으로 바로 연결됩니다.

## 함께 나온 게임
9월 8일 Google Play에 13종을 함께 냈습니다. 그중 10종은 앱인토스에도 같이 올라가 하루에 하나씩 소개해 왔고, 이 게임을 포함한 나머지 3종은 앱인토스 심사가 늦어져 9월 13일에 합류했습니다.

- [2048](/ko/posts/game-2048/) — 숫자를 합쳐 2048 타일을 만드는 퍼즐
- [지뢰찾기](/ko/posts/minesweeper/) — 숫자를 읽고 지뢰를 피하는 논리 퍼즐
- [솔리테어](/ko/posts/solitaire/) — 윈도우에서 하던 그 클론다이크
- [벽돌깨기](/ko/posts/breakout/) — 공을 튕겨 벽돌을 모두 깨는 아케이드
- [오목](/ko/posts/gomoku/) — 다섯 알을 먼저 잇는 사람이 이깁니다
- [뱀 게임](/ko/posts/snake/) — 먹이를 먹고 길어지는 클래식
- [숫자 퍼즐](/ko/posts/slide-puzzle/) — 타일을 밀어 순서대로 맞추는 15퍼즐
- [픽셀 탁구](/ko/posts/pixel-pong/) — AI 또는 친구와 1:1 탁구 대결
- [타워 쌓기](/ko/posts/tower-stack/) — 타이밍 맞춰 블록을 높이 쌓기
- [하늘 점프](/ko/posts/sky-jump/) — 발판을 밟고 끝없이 높이 오르기
- [가위바위보](/ko/posts/rock-paper-scissors/) — 패턴을 읽는 AI와 연승 대결

먼저 나온 6종의 이야기는 [미니게임 6종 동시 출시 회고](/ko/posts/six-games-retrospective/)에 정리해 두었습니다.

소식은 이 블로그와 [인스타그램(@fadongkwon.soft)](https://www.instagram.com/fadongkwon.soft/)에서 전해드립니다.

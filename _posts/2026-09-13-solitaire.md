---
title: 솔리테어 — 윈도우에서 하던 그 클론다이크
description: 익숙한 클론다이크 규칙에 자동 정리, 되돌리기, 오늘의 도전, 이어하기를 담은 솔리테어를 Google Play와 앱인토스에 출시했습니다
date: 2026-09-13 09:00:00 +0900
categories: [Products, Game]
tags: [game, android, 솔리테어, 카드게임, 클론다이크, 1인개발자]
pin: false
image:
  path: /assets/img/20260913_solitaire/feature.png
alt_url: /en/posts/solitaire/
---
## Info
> **Solitaire** is classic Klondike: the rules you already know, plus auto-collect, undo, a daily deal and auto-save. Works offline, no sign-up. 9 languages.
{: .prompt-info }

## 새로 배울 것이 없는 카드 게임
설명을 읽지 않아도 되는 게임이 하나쯤 있으면 좋겠다고 생각했습니다.
**규칙은 그대로 두고, 손이 덜 가게 만드는 것**만 했습니다.

- 🃏 **클래식 클론다이크** — 익숙한 규칙 그대로
- ✨ **자동 정리** — 올릴 수 있는 카드를 한 번에 위로 보냅니다
- ↩️ **되돌리기** — 막히면 한 수 물러설 수 있어요
- 📅 **오늘의 도전** — 매일 새 딜, 같은 판으로 겨룹니다
- 🏅 **기록** — 클리어 시간과 이동 수가 쌓입니다
- 💾 **이어하기** — 진행 중인 판은 자동 저장됩니다

완전 오프라인으로 동작하고, 회원가입·로그인 없이 바로 시작할 수 있습니다. 9개 언어를 지원합니다.

![솔리테어 홈](/assets/img/20260913_solitaire/shot-home.png){: w="360" }
![플레이 화면](/assets/img/20260913_solitaire/shot-play.png){: w="360" }

## Download
- Google Play: <https://play.google.com/store/apps/details?id=com.fadongkwon.solitaire>
- 앱인토스(토스 미니앱): <https://fadongkwon.com/toss/solitaire/> — 휴대폰에서 열면 토스 앱으로 바로 연결됩니다.

## 함께 나온 게임
9월 8일에 10종을 함께 출시했고, 하루에 하나씩 소개하고 있습니다. 지금까지 올린 글입니다.

- [2048](/posts/game-2048/) — 숫자를 합쳐 2048 타일을 만드는 퍼즐
- [지뢰찾기](/posts/minesweeper/) — 숫자를 읽고 지뢰를 피하는 논리 퍼즐

먼저 나온 6종의 이야기는 [미니게임 6종 동시 출시 회고](/posts/six-games-retrospective/)에 정리해 두었습니다.

소식은 이 블로그와 [인스타그램(@fadongkwon.soft)](https://www.instagram.com/fadongkwon.soft/)에서 전해드립니다.

---
title: 수학 몬스터 — 문제를 풀며 몬스터를 잡는 아이 수학 게임
description: 덧셈부터 나눗셈까지, 몬스터를 잡는 재미로 연산 연습을 이어가는 어린이 수학 학습 게임을 출시했습니다
date: 2026-08-25 21:00:00 +0900
categories: [Products, Game]
tags: [game, android, education, 수학, 수학공부, 유아교육, math, 몬스터, 1인개발자]
pin: false
image:
  path: /assets/img/20260825_math-monsters/feature.png
alt_url: /posts/math-monsters/
---
## Info
> **Math Monsters** is a math-practice game for kids: solve addition, subtraction,
> multiplication and division problems to catch monsters. Addition and subtraction
> (easy) are free; other operations unlock via one-time in-app purchase. No ads.
{: .prompt-info }

> **2026-09-05 업데이트**: 인앱 구매를 전부 없애고 모든 모드를 무료로 열었습니다. 자세한 이야기는 [유료 상품을 없앤 이유](/ko/posts/monsters-go-free/)에 적었습니다.
{: .prompt-tip }

## 몬스터를 잡으며 배우는 수학
아이가 수학 문제를 **풀 때마다 몬스터를 한 마리씩 잡는** 학습 게임입니다.
정답을 빠르고 정확하게 맞힐수록 좋은 기록이 남아, 아이 스스로 반복하게 됩니다.

- 👾 **연산 4종** — 덧셈 · 뺄셈 · 곱셈 · 나눗셈
- 🎚️ **난이도 3단계** — 쉬움 · 보통 · 어려움
- 🏆 **기록 저장** — 연산·난이도별 최고 기록으로 실력 변화 확인
- 🆓 모든 연산·난이도 무료 (출시 당시엔 덧셈·뺄셈 쉬움만 무료였고 나머지는 인앱 구매였으나 2026-09-05 전부 무료로 전환)

광고는 없습니다. 회원가입·로그인 없이 바로 시작할 수 있어요.

![수학 몬스터 홈](/assets/img/20260825_math-monsters/shot-home.png){: w="360" }
![플레이 화면](/assets/img/20260825_math-monsters/shot-play.png){: w="360" }

## Download
- Google Play: <https://play.google.com/store/apps/details?id=com.fadongkwon.math_monsters>
- 앱인토스: <https://fadongkwon.com/toss/math-monsters/> — 휴대폰에서 열면 토스 앱으로 바로 연결됩니다.

이 앱이 만들어진 뒷이야기 — 첫째가 구몬을 그만둔 뒤 아빠가 직접 연습 도구를 만들게 된 사연은 [구몬을 그만둔 날, 아빠는 수학 게임을 만들기 시작했다](/ko/posts/kumon-to-math-monsters/)에 적어뒀습니다.

소식은 이 블로그와 [인스타그램(@fadongkwon.soft)](https://www.instagram.com/fadongkwon.soft/)에서 전해드립니다.
